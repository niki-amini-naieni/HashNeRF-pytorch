...
def compute_alpha_weights(density, t_vals, dirs):
  ...
  dirs_norm = jnp.linalg.norm(dirs[Ellipsis, None, :], axis=-1)
  delta = t_dists * dirs_norm
...
def volumetric_rendering(rgbs,
  ...
  rendering['mu'] = rgbs
  rendering['pi'] = pi
  rendering['gamma'] = jnp.nan_to_num(gamma) + eps
  rendering['weights'] = jnp.nan_to_num(weights) + eps
  # for flipped reflection rays
  if pi_flip is not None:
    rendering['mu_flip'] = rgbs_flip
    rendering['pi_flip'] = pi_flip
    rendering['gamma_flip'] = jnp.nan_to_num(gamma_flip) + eps
    rendering['weights_flip'] = jnp.nan_to_num(weights_flip) + eps
    rendering['feature_dist'] = jnp.nan_to_num(feature_dist) + eps
    rendering['feature_dist_flip'] = jnp.nan_to_num(feature_dist_flip) + eps
    rendering['flip_mask'] = flip_mask
    rendering['viewdirs'] = viewdirs
...
def sample_along_rays(rng,
  ...
  means, covs = cast_rays(
      t_vals, origins, directions, radii, ray_shape, diag)
