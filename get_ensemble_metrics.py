...
def config_parser():
    ...
    parser.add_argument(
        "--iters", type=int, default=5000, help="number of training iterations"
    )
...
def test():
    ...
    ensemble_preds = np.zeros(
        (args.M, gts.shape[0], gts.shape[1], gts.shape[2], gts.shape[3])
    )
    ensemble_accs = np.zeros((args.M, gts.shape[0], gts.shape[1], gts.shape[2]))
    with torch.no_grad():
        ...
        for member_ind in range(args.M):
            ...
            ensemble_preds[member_ind, :, :, :, :] = rgbs
            ensemble_accs[member_ind, :, :, :] = accs

    preds = np.mean(ensemble_preds, axis=0)
    vars = np.var(ensemble_preds, axis=0)
    accs = np.mean(ensemble_accs, axis=0)
